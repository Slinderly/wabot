...
const pythonPath = 'python3'; // corregido para Railway
...
function saveLog(url, title, status, error = null) {
    ...
    if (fs.existsSync(LOGS_FILE)) {
        try { logs = JSON.parse(fs.readFileSync(LOGS_FILE, 'utf8')); } 
        catch (e) { logs = []; }
    }
    const newLog = {
        id: Date.now(),
...
app.get('/', (req, res) => {
    ...
    else res.send("⚠️ index.html no encontrado.");
});

app.get('/api/info', async (req, res) => {
    let qrImg = null;
    try { if (qrCodeData) qrImg = await QRCode.toDataURL(qrCodeData); } 
    catch (e) { console.error("Error generando QR", e); }

    let history = [];
    if (fs.existsSync(LOGS_FILE)) {
        try { history = JSON.parse(fs.readFileSync(LOGS_FILE, 'utf8')).slice(0, 10); } 
        catch(e) {}
    }

    res.json({ status: connectionStatus, qr: qrImg, logs: history, console: consoleBuffer.slice(0, 50) });
});
    ...
    try {
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (qr) {
                ...
                addConsoleLog("QR generado. Escanea con tu celular.");
            }
            ...
            if (connection === 'close') {
                ...
                addConsoleLog(`⚠️ Conexión cerrada. Reconectar?: ${shouldReconnect}`);
                
                if (shouldReconnect) setTimeout(startBot, 5000);
                else {
                    addConsoleLog("❌ Sesión cerrada permanentemente. Borrando auth...");
        ...
        sock.ev.on('messages.upsert', async ({ messages }) => {
            ...
            if (!handled && !m.key.remoteJid.endsWith('@g.us')) {
                await sock.sendMessage(jid, { 
                    text: "🎥 *WABOT - BOLIVIA*\n\nEnvíame un enlace de YouTube o TikTok para descargarlo." 
                });
            }
        });
